...
```mermaid
flowchart LR
    U[User (Browser)] -->|Inputs: destination, days, budget, styles, language| FE[Streamlit Frontend]
    FE --> PB[Prompt Builder]
    PB -->|Structured prompt| LLM[Groq Llama (LLM API)]
    LLM -->|Draft plan (Markdown)| ENRICH[SerpAPI Fetcher]
    ENRICH -->|Hotels/Attractions/Links JSON| MERGE[Plan Post-Processor]
    MERGE -->|Final plan (Markdown + links)| RENDER[UI Renderer]
    RENDER -->|View plan + Q&A| U
    RENDER -->|Export| PDF[PDF Exporter]
    PDF -->|Download .pdf| U

    %% Q&A path
    U -->|Question| QA[Q&A Handler]
    QA -->|Use current plan as context| LLM
    LLM -->|Answer (Markdown)| RENDER
...
Deliverable
Diagram illustrating the architecture and data flow (above).
...
- **Session State:** Travel plan stored in `st.session_state.travel_plan` for re-use in Q&A.  
